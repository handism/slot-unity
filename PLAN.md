...
- [ ] リールストリップ調整 → 再シミュレーション
- [ ] フリースピン込み RTP ≈ 94〜96% を確認
- [ ] `ReelStripData` アセットを確定値に更新

---

## フェーズ 6: 統合テスト・最終調整

- [ ] Play Mode テスト（`SpinFlowTests.cs`）
  - [ ] 通常スピン 1 回のエンドツーエンドフロー
  - [ ] フリースピン発動〜完走フロー
  - [ ] オートスピン中断テスト
  - [ ] ゲームオーバー → リセットフロー
- [ ] 手動テストチェックリスト
  - [ ] スピン中に早期停止ボタンが機能する
  - [ ] フリースピン中にオートスピンが自動停止する
  - [ ] ボーナスラウンドとフリースピン同時発動でボーナス優先
  - [ ] コイン 0 でスピンボタンが無効化される
  - [ ] BGM/SE ボリュームが再起動後も保存されている
  - [ ] ウィンドウリサイズ・フルスクリーン切り替えでレイアウトが崩れない
  - [ ] ペイテーブルの配当値が requirements.md の値と一致する
  - [ ] セーブデータ破損時にデフォルト値でフォールバックする
